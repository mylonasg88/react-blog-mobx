...
export default inject("blogStore")(
    observer(
        class AllPosts extends React.Component<*, { status: string }> {
            ...
            removePost(id: number) {
                ...
                this.props.blogStore.posts = this.props.blogStore.posts.filter(
                    post => post.id !== id
                );
            ...
            render() {
                const blogStore = this.props.blogStore;
                const blogList = blogStore.posts.map((post, i) => (
                    <li key={post.id}>
                        <NavLink to={{ pathname: "/post/" + post.id, pos: i }}>
                            {" "}
                            {post.id} {post.title}
                        </NavLink>
                    </li>
